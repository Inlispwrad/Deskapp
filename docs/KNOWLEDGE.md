...
> 长期共识，索引向。一行一条，不展开——详细内容在同目录里，这里只放指路牌。
...
- 记账/统计口径一旦定下就写进注释与 `ARCHITECTURE.md`，改口径等于改结论。
...
- **协议与接口的权威文档** → `PROTOCOL.md`（完整清单、`window.deskapp`、CLI、打包）
- **架构与设计取舍的权威文档** → `ARCHITECTURE.md`（选型理由、记账口径与边界、性能档位、安全姿态、已知限制）
...
- webapp 侧对接面 → `PROTOCOL.md#windowdeskapp--webapp-对接面`
