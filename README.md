...
高性能 webapp 桌面宿主。装载 webapp 之后，**用起来就是一个桌面应用，不是一个被遥控的浏览器**。

装载的 webapp 可能运行压力大、显存吃紧（浏览器 GPU 显存有上限，越线可能崩溃重启）。Deskapp 负责两件事：

1. **像应用**——抹掉一切浏览器痕迹，窗口身份由 webapp 自己声明；
2. **看得见**——把帧时序、显存占用、drawcall、进程资源、GPU 状态量出来，并在越线时喊出来。
...
除 `deskapp.json` 外，Deskapp 也读标准 [Web App Manifest]（`manifest.json`）。
